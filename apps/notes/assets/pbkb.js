...
pbkb.prototype.drawTextEntry = function () {
  ...
  bC.drawString('...', this.homeX - 84, this.homeY - 25);
  bC.drawRect(10, 5, 260, 200);
  (bC.drawString(bC.wrapString(this.assembledString, 240).join('\n'), 15, 10),
    bH.flip());
  bF.flip();
  bC.flip();
};
pbkb.prototype.input = function (d) {
  Pip.knob1Click(d);
  if (d == 0) {
    var ltr = this.alphabets[this.selectedAlpha][this.selectedLetter];
    if (ltr == 'UPR') {
      this.selectedAlpha = 1;
    } else if (ltr == 'NUM') {
      this.selectedAlpha = 2;
    } else if (ltr == 'LWR') {
      this.selectedAlpha = 0;
    } else if (ltr == 'DEL') {
      if ((this.cursor = 0)) {
        this.assembledString = this.assembledString.substring(
          0,
          this.assembledString.length - this.cursor - 1,
        );
      } else {
        var str1 = this.assembledString.substring(
          0,
          this.assembledString.length - this.cursor - 1,
        );
        var str2 = this.assembledString.substring(
          this.assembledString.length - this.cursor,
        );
        this.assembledString = str1 + str2;
      }
    } else if (ltr == 'END') {
      this.finished = true;
...
exports.initKeyboard = function () {
  let o = new pbkb();
  o._onKnob1 = o.input.bind(o);
  Pip.removeAllListeners('knob1');
  Pip.on('knob1', o._onKnob1);
  return o;
};
